...
export function useGameplayRuntime(
  ...
  eventBatch: number,
  fakeMedia: boolean,
  runtimeAudioEnabled: boolean,
  onTimerExpired: () => void,
): RuntimeResult {
  const audioEnabled = useSettingsStore((state) => state.audioEnabled);
  const suspenseVolume = useSettingsStore((state) => state.suspenseVolume);
  const effectsVolume = useSettingsStore((state) => state.effectsVolume);
  const shouldEnableAudio = runtimeAudioEnabled && audioEnabled;
  ...
  useEffect(() => {
    runtime.audio.setSuspenseVolume(suspenseVolume);
    runtime.audio.setEffectsVolume(effectsVolume);
    if (shouldEnableAudio) {
      void runtime.audio.preload();
    } else {
      runtime.audio.stopSuspense();
    }
  }, [effectsVolume, runtime, shouldEnableAudio, suspenseVolume]);
  ...
  useEffect(() => {
    events.forEach((event) => {
      if (event.type === 'SUSPENSE_START_REQUESTED') {
        if (shouldEnableAudio) void runtime.audio.playSuspense('countdown');
      } else if (event.type === 'SUSPENSE_STOP_REQUESTED') {
        runtime.audio.stopSuspense();
      } else if (event.type === 'SCORE_CONFIRMED' && shouldEnableAudio) {
        const score = session.activeTurn?.score;
        const awardedPoints =
          (score?.primary.finalAwardedPoints ?? 0) + (score?.steal?.finalAwardedPoints ?? 0);
        void runtime.audio.playEffect(awardedPoints > 0 ? 'correct' : 'wrong');
  ...
  }, [eventBatch, events, runtime, session, shouldEnableAudio]);
